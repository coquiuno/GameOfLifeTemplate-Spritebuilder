...
-(void)setupGrid{
    ...
    // divide the grid's size by the number of colums/rows to figure out the right width and height for each cell
...
-(void)touchBegan:(CCTouch *)touch withEvent:(CCTouchEvent *)event{
    ...
    //invert it's state - kill it if it is alive and vice versa
...
for (int i = 0; i < [_gridArray count]; i++){
    ...
    for (int j = 0; j < [_gridArray[i] count]; j++){
        ...
        for (int x = (i - 1); x <= (i + 1); x++){
            ...
            // go through the column through the left of the current cell, the column the cell is in, and the column to the right of the current cell
